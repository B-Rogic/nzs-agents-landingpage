...
const AnimatedButton = ({notTextHover, className, text, textClassname, hover, border, add, padding}: Props) => {
  ...
  return (
    <button ref={buttonRef} className={`group relative flex ${padding || 'p-3'} flex-col transition-bg duration-200 ${hover || "bg-majororange hover:bg-white text-majorgreen"} ${border ? "" : "border border-white"} font-bold overflow-hidden ${className} cursor-pointer`}>
